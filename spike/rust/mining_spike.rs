// Lode mining-core spike (Rust). Throwaway: language comparison + RFC-0003 sanity.
// std-only, no crates. Build: rustc -O mining_spike.rs -o mining_spike
...
struct Template { tokens: Vec<String>, count: u64 }
...
fn main() {
    ...
    // Pre-tokenize + mask the sample once (ingestion/enrichment is not what we benchmark here).
    let masked: Vec<Vec<String>> = SAMPLE.iter()
        .map(|line| line.split_whitespace().map(mask).collect())
        .collect();

    let mut templates: Vec<Template> = Vec::new();
    let mut index: HashMap<(usize, String), Vec<usize>> = HashMap::new();

    let start = Instant::now();
    for i in 0..n {
        let toks = &masked[i % masked.len()];
        let len = toks.len();
        let pref_end = d.min(len);
        let key = (len, toks[..pref_end].join("\u{1}"));
        ...
        for &ti in cands.iter() {
            ...
            if s >= best_sim { best_sim = s; best = Some(ti); }
        }
        match best {
            Some(ti) => {
                let t = &mut templates[ti];
                for j in 0..len {
                    if t.tokens[j] != toks[j] && t.tokens[j] != "<*>" { t.tokens[j] = "<*>".into(); }
                }
                t.count += 1;
            }
            None => {
                let id = templates.len();
                templates.push(Template { tokens: toks.clone(), count: 1 });
                cands.push(id);
            }
        }
    }
    ...
    // Deterministic template-set hash (FNV-1a over sorted patterns, counts excluded).
    let mut patterns: Vec<String> = templates.iter().map(|t| t.tokens.join(" ")).collect();
    patterns.sort();
    ...
    let mut out: Vec<String> = templates.iter().map(|t| format!("{:>10}  {}", t.count, t.tokens.join(" "))).collect();
    out.sort();
